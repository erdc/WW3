...
CONTAINS
  ...
  SUBROUTINE W3GRID()
    ...
    IF (EXPSCHEME .AND. IMPSCHEME) THEN
      WRITE (NDSE,*) 'EXPLICIT AND IMPLICIT SCHEMES ARE BOTH SELECTED'
      CALL EXTCDE ( 19 )
    END IF
    IF (EXPSCHEME) EXPFSN = .TRUE.
    IF (EXPTOTAL) EXPFSN = .FALSE.
    IF (IMPSCHEME) THEN
      IMPTOTAL = .TRUE.
      IMPREFRACTION = .TRUE.
      IMPFREQSHIFT = .TRUE.
