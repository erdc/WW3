...
!      REAL, INTENT(IN)        :: CG(NK)   We don't use CG in this formulation TJH
      REAL                    :: CVEG
!      LOGICAL, INTENT(OUT)    :: LBREAK   We are not using LBREAK at this time TJH
      INTEGER                 :: ITH, IK, IWB
...
!      REAL                    :: VNV   = 200 ! veg. density 
!      REAL                    :: VLTH  = 0.415 ! vegetation height  
...
!!         ENDIF
      ...
      IF (INFLAGS2(-8)) THEN
!!        IF (LPDLIB) THEN
!!          VCD = VEGCD(iplg(IX), IY)
!!         ELSE
           VCD = VEGCD(IX, IY)
!!         ENDIF
      ELSE
        IF ( IAPROC .EQ. NAPERR )                &
        WRITE (NDSE,1001) 'VEGETATION DRAG COEFFICIENT'
        CALL EXTCDE(2)
      END IF
...
!     write(*,*) 'SizeEMEAN',SIZE(EMEAN)
!     write(*,*) SIZE(A), SIZE(SQRT(EMEAN)), SIZE(WNMEAN), SIZE(VALPHAD)
